...
/**
 * MerchantApplicationRequest - DTO for merchant application submission
 * <p>
 * This DTO contains all the required fields for merchant registration as per
 * RBI Payment Aggregator guidelines. It includes comprehensive validation
 * annotations to ensure data integrity and compliance.
 * <p>
 * VALIDATION FEATURES:
 * - Jakarta validation annotations for all fields
 * - Format validation for PAN, GSTIN, IFSC codes
 * - Email and phone number format validation
 * - Date validation (past dates for DOB, past/present for incorporation)
 * - Size constraints to prevent oversized data
 * - Required field validation for mandatory information
 * <p>
 * SECTIONS INCLUDED:
 * 1. Business/Entity Information (legal name, type, category, etc.)
 * 2. Authorized Signatory KYC (name, DOB, designation, contact, ID proof)
 * 3. Bank Account Details (settlement account information)
 * 4. Transaction Profile (business volume, risk category, policies)
 * 5. KYC Document Types (list of document types to be uploaded)
 * <p>
 * COMPLIANCE:
 * - RBI PA guidelines compliant field structure
 * - PCI-DSS compliant validation patterns
 * - DPDP Act compliant for personal data validation
 * - GDPR compliant for data validation requirements
 * <p>
 * USAGE:
 * - Used as request body for merchant application API
 * - Validated automatically by Spring Boot validation framework
 * - Mapped to entity with sensitive fields encrypted before storage
 */
public class MerchantApplicationRequest {
    ...
    // Upload document types expected (filenames sent separately via multipart)
    private List<String> kycDocumentTypes;
    ...
    public String getSignatoryGovtIdNumber() {
        return signatoryGovtIdNumber;
    }

    public void setSignatoryGovtIdNumber(String signatoryGovtIdNumber) {
        this.signatoryGovtIdNumber = signatoryGovtIdNumber;
    }
    ...
    public List<String> getKycDocumentTypes() {
        return kycDocumentTypes;
    }

    public void setKycDocumentTypes(List<String> kycDocumentTypes) {
        this.kycDocumentTypes = kycDocumentTypes;
    }
}
